...
internal fun ColumnScope.RestoreNovaBackupContent(
    ...
    PreferenceGroup(
        heading = stringResource(id = R.string.restore_nova_backup),
    ) {
        Text(
            text = stringResource(
                R.string.nova_grid_info,
                state.info.columns,
                state.info.rows,
                state.info.hotseatCount,
            ),
            modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
        )
        if (state.info.appCount > 0) {
            Text(
                text = stringResource(R.string.nova_app_count, state.info.appCount),
                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
            )
        }
        if (state.info.widgetCount > 0) {
            Text(
                text = stringResource(R.string.nova_widget_count, state.info.widgetCount),
                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
            )
        }
        if (state.info.folderCount > 0) {
            Text(
                text = stringResource(R.string.nova_folder_count, state.info.folderCount),
                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
            )
        }
        if (state.info.shortcutCount > 0) {
            Text(
                text = stringResource(R.string.nova_shortcut_count, state.info.shortcutCount),
                modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
            )
        }
